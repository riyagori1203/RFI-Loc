...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def plotfile(self, pos_dict, delay_dict):
        ...
        for i in range(0, l):
            ...
            xyp[2, :] = (xyp[2, :] - np.min(xyp[2, :])) / \
                (np.max(xyp[2, :]) - np.min(xyp[2, :]))

            lc = LineCollection(segments, linewidths=0.5,
                                colors='k', alpha=(xyp[2, :]))
            self.axes.add_collection(lc)
            print(segments)
            if(xyp[2, 0] > 0.1 and xyp[2, 0] < 0.4):
                lc = LineCollection(segments, linewidths=0.5,
                                    colors='k', alpha=xyp[2, :])
                self.axes.add_collection(lc)
            else:
                lc = LineCollection(segments, linewidths=0.5, colors='k', alpha=)
                self.axes.add_collection(lc)
            colors1 = [colors.to_rgba(c)
                       for c in plt.rcParams['axes.prop_cycle'].by_key()['color']]
            col = collections.LineCollection(segments)
            col.set_color(colors1)
            self.axes.add_collection(col, autolim=True)
            # self.axes.plot(alpha=0.1)

            self.axes.plot(xyp[0, :], xyp[1, :], c=viridis_r(
                norm(strength)), lw=1, picker=5, label=i, pickradius=5, alpha=(xyp[2, i]))
    ...
    def load_file(self):
        ...
        if self.file_path[0]:
            ...
            if fileType == "npz":
                ...
                # self.observation.read_ms(self.file_path[0])
                ...
                # self.observation.antenna_positions[0:
                #                                    num_ants] = fh["antenna_positions"]
