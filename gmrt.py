...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def plotfile(self, pos_dict,delay_dict):
        ...
        for i in range(0,l):
            ...
            segments = [np.vstack((xyp[0, :], xyp[1, :])).T for i  in range(l)]
            # wt=Normalize(vmin=np.min(xyp[2,:]), vmax=np.max(xyp[2,:]))
            lc = matplotlib.collections.LineCollection(segments, picker=5,label=i, lw=xyp[2,:], colors=[colorConverter.to_rgba(i) for i in 'bgrcmyk'])
